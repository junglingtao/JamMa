...
class Loss(nn.Module):
    ...
    def compute_fine_matching_loss(self, data):
        ...
        loss_pos = (
            -alpha
            * torch.pow(1 - conf_matrix_fine[pos_mask], gamma)
            * (conf_matrix_fine[pos_mask]).log()
        )
        loss_neg = (
            -alpha
            * torch.pow(conf_matrix_fine[neg_mask], gamma)
            * (1 - conf_matrix_fine[neg_mask]).log()
        )
    ...
    def compute_coarse_loss(self, data, weight=None):
        ...
        loss_pos = (
            -alpha
            * torch.pow(1 - conf_matrix_0_to_1[pos_mask], gamma)
            * (conf_matrix_0_to_1[pos_mask]).log()
        )
        loss_pos += (
            -alpha
            * torch.pow(1 - conf_matrix_1_to_0[pos_mask], gamma)
    ...
    def forward(self, data):
        ...
        if loss_sub is not None:
            ...
            assert self.training is False
            loss_scalars.update({"loss_sub": torch.tensor(1.0)})  # 1 is the upper bound
